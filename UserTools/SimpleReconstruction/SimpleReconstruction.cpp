...
bool SimpleReconstruction::GetANNIEEventVariables(){
  ...
  for(int tracki=0; tracki<numtracksinev; tracki++){
    ...
    NumClusterTracks+=1;
    if (verbosity > 4) Log("SimpleReconstruction Tool: NumClusterTracks: " + std::to_string(NumClusterTracks), vv_debug, verbosity);
  }
...
bool SimpleReconstruction::RecoTankExitPoint(std::vector<int> clusterid_vec){
    Log("SimpleReconstruction Tool: ClusterID Vector size: " + std::to_string(clusterid_vec.size()), vv_debug, verbosity);
  ...
  for (int i_clusterid=0; i_clusterid < (int)clusterid_vec.size(); i_clusterid++) {
    ...
    for (int i = 0; i < (int) fMRDTrackEventID.size(); i++){
        Log("MRD cluster ID: " + to_string(fMRDTrackEventID.at(i)) + "; searching for " + to_string(i_clusterid), v_debug, verbosity);
