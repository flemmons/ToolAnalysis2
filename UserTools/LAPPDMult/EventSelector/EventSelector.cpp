...
bool EventSelector::Execute(){
  ...
  auto has_LAPPDMult = m_data->Stores.at("RecoEvent")->Get("LAPPDsHit", fLAPPDsHit);
  std::cout << "fLAPPDsHit: " << *fLAPPDsHit << endl;
  if (not has_LAPPDMult) {
      ...
      Log("EventSelector Tool: # of LAPPDs hit in this event: " + std::to_string(*fLAPPDsHit), v_debug, verbosity);
  }
  ...
  std::cout << "RecoPDGCut bypassed.  We're fine.  Somehow.\n";
  ...
  if(fNHitCut){
    ...
    if(!HasEnoughHits || *fLAPPDsHit < fLAPPDMultMin) fEventFlagged |= EventSelector::kFlagNHit;
  }
