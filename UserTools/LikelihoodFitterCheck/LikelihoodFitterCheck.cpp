...
bool LikelihoodFitterCheck::Execute(){
  ...
  for(int j=0;j<100;j++) {
    ...
    if(ifPlot2DFOM) {
      ...
      for(int k=0; k<100; k++) {
        for(int m=0; m<200; m++) {
        	seedX = trueVtxX - 50*dx_trans + k*dx_trans - 10*dx_para + m*dx_para;
        	seedY = trueVtxY - 50*dy_trans + k*dy_trans - 10*dy_para + m*dy_para;
        	seedZ = trueVtxZ - 50*dz_trans + k*dz_trans - 10*dz_para + m*dz_para;
        	seedT = trueVtxT;
