...
bool PhaseIITreeMakerDOE::Execute(){
  ...
  if (!annie_event) {
    Log("Error: The PhaseITreeMakerDOE tool could not find the RecoEvent Store", v_error, verbosity);
    return false;
  }
  ...
  if(!get_evtstatus) {
    Log("Error: The PhaseITreeMakerDOE tool could not find the Event selection status", v_error, verbosity);
    return false;	
  }
