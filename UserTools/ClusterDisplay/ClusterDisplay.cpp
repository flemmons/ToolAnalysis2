...
ClusterDisplay::ClusterDisplay():Tool(){}
...
bool ClusterDisplay::Initialise(std::string configfile, DataModel &data){
  ...
  m_variables.Get("convexHull",convex_hull);

  Log("ClusterDisplay tool: Initialising",v_message,verbose);
  ...
  if (histogram_config!="None"){
    ...
    while (!file_hist.eof()){
      ...
      if (temp_string=="pmt_Qmax") pmt_Qmax = temp_value;
        ...
        Log("ClusterDisplay tool: Error while reading in histogram configuration file "+histogram_config+": Cannot parse argument "+temp_string+", please look up the available options!",v_error,verbose);
      }
    }
    file_hist.close();
  }
  ...
  std::string logmessage = "ClusterDisplay tool: EventList size: "+std::to_string(ev_list.size());
  Log(logmessage,v_message,verbose);

  //---------------------------------------------------
  // ------Print chosen settings for ClusterDisplay------
  //---------------------------------------------------
  ...
  if (lappds_selected){ 
    ...
    std::string prefix = "./configfiles/ClusterDisplay/";
    ...
    while (!file_lappd.eof()){
      ...
      if (!file_lappd.eof()) {
        ...
        Log("ClusterDisplay tool: Emplacing user-specified lappd id "+std::to_string(temp_lappd)+" to active lappd settings.",v_message,verbose);
      }
    }
    file_lappd.close();
  }
  ...
  logmessage = "ClusterDisplay tool: Using detector version "+std::to_string(detector_version);
  Log(logmessage,v_message,verbose);
  ...
  if (verbose > 0) std::cout <<"ClusterDisplay: Num Tank PMTs: "<<n_tank_pmts<<", num MRD PMTs: "<<n_mrd_pmts<<", num Veto PMTs: "<<n_veto_pmts<<", num LAPPDs: "<<n_lappds<<std::endl;
  ...
  for (std::map<unsigned long,Detector*>::iterator it = Detectors->at("LAPPD").begin();
    ...
    logmessage = "ClusterDisplay tool: Looping over LAPPD detector key "+std::to_string(alappd->GetDetectorID());
  	Log(logmessage,v_debug,verbose);
  	lappd_detkeys.push_back(alappd->GetDetectorID());
   }
   max_num_lappds = lappd_detkeys.size();
   logmessage = "ClusterDisplay tool: Number of LAPPDs: "+std::to_string(max_num_lappds);
   Log(logmessage,v_message,verbose);
    ...
    for(auto&& auserid : active_lappds_user){
      if (!isData){
        ...
        if(lappd_tubeid_to_detectorkey.count(auserid.first)==0){
          logmessage = "ClusterDisplay tool: No detectorkey found for user-provided LAPPDID "+std::to_string(auserid.first)+"!";
  ...
  for (std::map<unsigned long,Detector*>::iterator it  = Detectors->at("Tank").begin();
    ...
    logmessage = "ClusterDisplay tool: DetectorID = "+std::to_string(detkey)+", position = ("+std::to_string(position_PMT.X())+","+std::to_string(position_PMT.Y())+","+std::to_string(position_PMT.Z())+"), rho= "+std::to_string(sqrt(x_pmt.at(detkey)*x_pmt.at(detkey)+z_pmt.at(detkey)*z_pmt.at(detkey)))+", y = "+std::to_string(y_pmt.at(detkey));
    Log(logmessage,v_debug,verbose);
  ...
  logmessage = "ClusterDisplay tool: Properties of the detector configuration: "+std::to_string(n_veto_pmts)+" veto PMTs, "+std::to_string(n_mrd_pmts)+" MRD PMTs, "+std::to_string(n_tank_pmts)+" Tank PMTs, and "+std::to_string(n_lappds)+" LAPPDs.";
  Log(logmessage,v_message,verbose);
  logmessage = "ClusterDisplay tool: Max y (Tank PMTs): "+std::to_string(max_y)+", min y (Tank PMTs): "+std::to_string(min_y);
  Log(logmessage,v_message,verbose);
  ...
  Log("ClusterDisplay tool: Reading in MRD information...",v_message,verbose);
  ...
  for (std::map<unsigned long,Detector*>::iterator it  = Detectors->at("MRD").begin();
    ...
    Log("ClusterDisplay tool: Reading in MRD channel, detkey "+std::to_string(detkey)+", x = "+std::to_string(xmin)+"-"+std::to_string(xmax)+", y = "+std::to_string(ymin)+"-"+std::to_string(ymax)+", z = "+std::to_string(zmin)+"-"+std::to_string(zmax)+")",v_debug,verbose);
  ...
  Log("ClusterDisplay tool: Read in MRD PMT information: xmin = "+std::to_string(min_mrd_x)+", ymin = "+std::to_string(min_mrd_y)+", zmin = "+std::to_string(min_mrd_z)+", xmax = "+std::to_string(max_mrd_x)+", ymax = "+std::to_string(max_mrd_y)+", zmax = "+std::to_string(max_mrd_z),v_message,verbose);
  ...
  if (use_tapplication){
    Log("ClusterDisplay tool: Initializing TApplication",v_message,verbose);
    int myargc = 0;
    char *myargv[] {(char*) "options"};
    app_event_display = new TApplication("app_event_display",&myargc,myargv);
    Log("ClusterDisplay tool: TApplication running.",v_message,verbose);
  }
  ...
  if (output_format == "root") {
    ...
    root_file->mkdir("prompt");
    root_file->mkdir("prompt/Muon");
    root_file->mkdir("prompt/Neutron");
    root_file->mkdir("prompt/Proton");
    root_file->mkdir("prompt/other");
    root_file->mkdir("delayed");
    root_file->mkdir("delayed/Muon");
    root_file->mkdir("delayed/Neutron");
    root_file->mkdir("delayed/Proton");
    root_file->mkdir("delayed/other");
  }
...
bool ClusterDisplay::Execute(){

  Log("ClusterDisplay tool: Executing.",v_message,verbose);

  int annieeventexists = m_data->Stores.count("ANNIEEvent");
  if(!annieeventexists){ 
    Log("ClusterDisplay tool: No ANNIEEvent store!",v_error,verbose); 
    return false;
  }
  ...
  if(not get_ok) { Log("ClusterDisplay tool: Error retrieving EventNumber, true from ANNIEEvent!",v_error,verbose); return false;}
  get_ok = m_data->Stores["ANNIEEvent"]->Get("RunNumber",runnumber);
  if(not get_ok){ Log("ClusterDisplay tool: Error retrieving RunNumber, true from ANNIEEvent!",v_error,verbose); return false;}
  //get_ok = m_data->Stores["ANNIEEvent"]->Get("SubRunNumber",subrunnumber);
  //if(not get_ok){ Log("ClusterDisplay tool: Error retrieving SubRunNumber, true from ANNIEEvent!",v_error,verbose); return false;}
  ...
  if (isData) {
    ...
    if(not get_ok){ Log("ClusterDisplay tool: Error retrieving Hits, true from ANNIEEvent!",v_error,verbose);}
    ...
    if(not get_mrd){ Log("ClusterDisplay tool: Error retrieving TDCData (Data), true from ANNIEEvent!",v_error,verbose);}
    ...
    //if(not get_ok){ Log("ClusterDisplay tool: Error retrieving LAPPDHits, true from ANNIEEvent!",v_error,verbose);}
    get_ok = m_data->Stores["ANNIEEvent"]->Get("RunType",RunType);
    if(not get_ok){ Log("ClusterDisplay tool: Error retrieving RunType, true from ANNIEEvent! ",v_error,verbose); return false;}
    get_ok = m_data->Stores["ANNIEEvent"]->Get("EventTimeTank",EventTankTime);
    if(not get_ok){ Log("ClusterDisplay tool: Error retrieving EventTimeTank, true from ANNIEEvent! ",v_error,verbose); return false;}
    
    get_ok = m_data->Stores["ANNIEEvent"]->Get("EventTimeMRD",EventTimeMRD);
    if (verbose > 2) std::cout <<"ClusterDisplay tool: EventTimeTank: "<<EventTankTime<<", EventTimeMRD: "<<EventTimeMRD.GetNs()<<", difference: "<<double(EventTimeMRD.GetNs())-double(EventTankTime)<<std::endl;

    std::string MRDTriggertype;
    get_ok = m_data->Stores.at("ANNIEEvent")->Get("MRDTriggerType",MRDTriggertype);
    if (verbose > 2) std::cout <<"ClusterDisplay tool: MRDTriggertype is: "<<MRDTriggertype<<std::endl;
  }
  ...
  if (!isData){
    ...
    if(not get_ok){ Log("ClusterDisplay tool: Error retrieving MCParticles, true from ANNIEEvent!",v_error,verbose); return false;}
    get_ok = m_data->Stores["ANNIEEvent"]->Get("MCHits", MCHits);
    if(not get_ok){ Log("ClusterDisplay tool: Error retrieving MCHits, true from ANNIEEvent!",v_error,verbose); return false;}
    get_ok = m_data->Stores["ANNIEEvent"]->Get("TDCData",TDCData);  // a std::map<ChannelKey,vector<TDCHit>>
    if(not get_ok){ Log("ClusterDisplay tool: Error retrieving TDCData, true from ANNIEEvent!",v_error,verbose); return false;}
    get_ok = m_data->Stores["ANNIEEvent"]->Get("MCLAPPDHits",MCLAPPDHits);
    if(not get_ok){ Log("ClusterDisplay tool: Error retrieving MCLAPPDHits, true from ANNIEEvent!",v_error,verbose); return false;} 
    
    //EventTime currently not implemented in data
    get_ok = m_data->Stores["ANNIEEvent"]->Get("EventTime",EventTime);
    if(not get_ok){ Log("ClusterDisplay tool: Error retrieving EventTime, true from ANNIEEvent!",v_error,verbose); return false;}
    ...
    if(not get_ok){ Log("ClusterDisplay tool: Error retrieving BeamStatus, true from ANNIEEvent!",v_error,verbose); return false;}
    get_ok = m_data->Stores["ANNIEEvent"]->Get("TriggerData",TriggerData);
    if(not get_ok){ Log("ClusterDisplay tool: Error retrieving TriggerData, true from ANNIEEvent!",v_error,verbose); return false;}
    ...
    if(not get_ok){ Log("ClusterDisplay tool: Error retrieving MCEventNum, true from ANNIEEvent!",v_error,verbose); return false;}
    get_ok = m_data->Stores["ANNIEEvent"]->Get("MCTriggernum",MCTriggernum);
    if(not get_ok){ Log("ClusterDisplay tool: Error retrieving MCTriggernum, true from ANNIEEvent!",v_error,verbose); return false;}
    ...
    if(not get_ok){ Log("ClusterDisplay Tool: Error retrieving TrueVertex,true from RecoEvent!",v_error,verbose); return false; }
    get_ok = m_data->Stores.at("RecoEvent")->Get("TrueStopVertex",TrueStopVertex);
    if(not get_ok){ Log("ClusterDisplay Tool: Error retrieving TrueStopVertex,true from RecoEvent!",v_error,verbose); return false; }
    get_ok = m_data->Stores.at("RecoEvent")->Get("TrueMuonEnergy",TrueMuonEnergy);
    if(not get_ok){ Log("ClusterDisplay Tool: Error retrieving TrueMuonEnergy,true from RecoEvent!",v_error,verbose); return false; }
    get_ok = m_data->Stores.at("RecoEvent")->Get("NRings",nrings);
    if(not get_ok){ Log("ClusterDisplay Tool: Error retrieving NRings, true from RecoEvent!",v_error,verbose); return false; }
    get_ok = m_data->Stores.at("RecoEvent")->Get("IndexParticlesRing",particles_ring);
    if(not get_ok){ Log("ClusterDisplay Tool: Error retrieving IndexParticlesRing, true from RecoEvent!",v_error,verbose); return false; }
  }
  ...
  if (EventType == "RecoEvent"){
    get_ok = m_data->Stores.at("RecoEvent")->Get("RecoClusters", RecoClusters);
    if (not get_ok) { Log("ClusterDisplay Tool: Error retrieving Clusters,true from RecoEvent!", v_error, verbose); return false; }
    //get_ok = m_data->Stores.at("RecoEvent")->Get("RecoDigit",RecoDigits);
    //if(not get_ok){ Log("ClusterDisplay Tool: Error retrieving RecoDigit,true from RecoEvent!",v_error,verbose); return false; }
    get_ok = m_data->Stores.at("RecoEvent")->Get("EventCutStatus",EventCutStatus);
    if(not get_ok){ Log("ClusterDisplay Tool: Error retrieving EventCutStatus,true from RecoEvent!",v_error,verbose); return false; }
    Log("RecoCluster size: "+to_string(RecoClusters->size()),v_debug,verbose);
  }
  ...
  if (draw_cluster){
    ...
    if (not get_ok) { Log("ClusterDisplay Tool: Error retrieving ClusterMap from CStore, did you run ClusterFinder beforehand?",v_error,verbose); return false; }
    get_ok = m_data->CStore.Get("ClusterMapDetkey",m_all_clusters_detkey);
    if (not get_ok) { Log("ClusterDisplay Tool: Error retrieving ClusterMapDetkey from CStore, did you run ClusterFinder beforehand?",v_error,verbose); return false; }
  }
  //Get MRD Cluster information (from TimeClustering tool)
  if (draw_cluster_mrd){
    get_ok = m_data->CStore.Get("MrdTimeClusters",MrdTimeClusters);
    if (not get_ok) { Log("ClusterDisplay Tool: Error retrieving MrdTimeClusters map from CStore, did you run TimeClustering beforehand?",v_error,verbose); return false; }
    if (MrdTimeClusters.size()!=0){
      get_ok = m_data->CStore.Get("MrdDigitTimes",MrdDigitTimes);
      if (not get_ok) { Log("ClusterDisplay Tool: Error retrieving MrdDigitTimes map from CStore, did you run TimeClustering beforehand?",v_error,verbose); return false; }
      get_ok = m_data->CStore.Get("MrdDigitChankeys",mrddigitchankeysthisevent);
      if (not get_ok) { Log("ClusterDisplay Tool: Error retrieving MrdDigitChankeys, did you run TimeClustering beforehand",v_error,verbose); return false;}
    }
  }
  ...
  if (!isData){
    for (unsigned int i_trigger=0;i_trigger<TriggerData->size();i_trigger++){
      logmessage = "ClusterDisplay tool: Trigger name: "+TriggerData->at(i_trigger).GetName()+", Trigger time: "+std::to_string(TriggerData->at(i_trigger).GetTime().GetNs());
      Log(logmessage,v_message,verbose);
    ...
    if (not get_ok) { Log("ClusterDisplay Tool: Error retrieving TriggerWord, was the processing done with CTC information?",v_error,verbose); return false;}
  }

  if (verbose > 2) std::cout <<"ClusterDisplay tool: TriggerWord: "<<triggerword<<std::endl;

  //---------------------------------------------------------------
  //---------------Current event to be processed? -----------------
  //---------------------------------------------------------------

  logmessage = "ClusterDisplay tool: Event number "+std::to_string(evnum);
  Log(logmessage,v_message,verbose);
  ...
  Log("ClusterDisplay tool: Event number check passed.",v_debug,verbose);
  ...
  out_file_dir = out_file+plots;
  filename_evdisplay = out_file_dir+str_evdisplay+str_Run+str_runnumber+str_Ev+str_evnumber+str_png;
  filename_pmts = out_file_dir+str_evdisplay+str_Run+str_runnumber+str_Ev+str_evnumber+str_pmts+str_png;
  filename_pmts2D = out_file_dir+str_evdisplay+str_Run+str_runnumber+str_Ev+str_evnumber+str_pmts2D+str_png;
  filename_lappds = out_file_dir+str_evdisplay+str_Run+str_runnumber+str_Ev+str_evnumber+str_lappds+str_png;
  ...
  if (!isData){
    ...
    Log("ClusterDisplay tool: Loop through MCParticles.",v_message,verbose);
    for(unsigned int particlei=0; particlei<mcparticles->size(); particlei++){

      MCParticle aparticle = mcparticles->at(particlei);
      std::string logmessage = "ClusterDisplay tool: Particle # "+std::to_string(particlei)+", parent ID = "+std::to_string(aparticle.GetParentPdg())+", pdg = "+std::to_string(aparticle.GetPdgCode())+", flag = "+std::to_string(aparticle.GetFlag());
      Log(logmessage,v_message,verbose);
      ...
      if ((aparticle.GetPdgCode() == 11 || aparticle.GetPdgCode() == 13)){    //primary particle for Cherenkov tracking should be muon or electron
        ...
        truedir_y = truedir.Y();
        truedir_z = truedir.Z();
        std::string logmessage = "ClusterDisplay tool: True vtx: ( "+std::to_string(truevtx_x)+", "+std::to_string(truevtx_y)+", "+std::to_string(truevtx_z)+" ), true dir: ("+std::to_string(truedir_x)+", "+std::to_string(truedir_y)+", "+std::to_string(truedir_z)+")";
        Log(logmessage, v_message, verbose);
        ...
        if ((truevtx_y< min_y) || (truevtx_y>max_y) || sqrt(truevtx_x*truevtx_x+truevtx_z*truevtx_z)>tank_radius){
          Log("ClusterDisplay tool: Event vertex outside of Inner Structure! Don't plot projected vertex and ring...",v_message,verbose);
	  Log("ClusterDisplay tool: Vtx_x = "+to_string(truevtx_x)+", vtx_y = "+to_string(truevtx_y)+", vtx_z = "+to_string(truevtx_z),v_message,verbose);
          draw_vertex_temp=false;
          draw_ring_temp=false;
        }
        break;
    ...
    if (!isData){
      ...
      if (MCHits){
        ...
        for(std::pair<unsigned long, std::vector<MCHit>>&& apair : *MCHits){
          ...
          Log("ClusterDisplay tool: Loop over MCHits: Chankey = "+std::to_string(chankey),v_debug,verbose);
          Detector* thistube = geom->ChannelToDetector(chankey);
          unsigned long detkey = thistube->GetDetectorID();
          Log("ClusterDisplay tool: Loop over MCHits: Detkey = "+std::to_string(detkey),v_debug,verbose);
        ...
        Log("ClusterDisplay tool: No MCHits!",v_warning,verbose);
        vectsize = 0;
      }
      logmessage = "ClusterDisplay tool: MCHits size = "+std::to_string(vectsize);
      Log(logmessage,v_message,verbose);
      ...
      if (!draw_cluster && Hits){
        ...
        for(std::pair<unsigned long, std::vector<Hit>>&& apair : *Hits){
          unsigned long chankey = apair.first;
          Log("ClusterDisplay tool: Loop over Hits: Chankey = "+std::to_string(chankey),v_debug,verbose);
          Detector* thistube = geom->ChannelToDetector(chankey);
          unsigned long detkey = thistube->GetDetectorID();
          Log("ClusterDisplay tool: Loop over Hits: Detkey = "+std::to_string(detkey),v_debug,verbose);
          if (thistube->GetDetectorElement()=="Tank"){
            if(thistube->GetTankLocation()=="OD") continue;             //don't plot OD PMTs (they are not included in the final design of ANNIE)
        ...
        Log("ClusterDisplay tool: Clustersize of m_all_clusters: "+std::to_string(clustersize),v_message,verbose);
      ...
      Log("ClusterDisplay tool: No Hits!",v_warning,verbose);
      vectsize = 0;
    }
  }
      ...
      Log("Total Clusters found in this event: "+to_string(RecoClusters->size()),v_debug,verbose);

      for(int i_cluster = 0;i_cluster<RecoClusters->size(); i_cluster++){
          if(!convex_hull) RecoDigits=RecoClusters->at(i_cluster)->GetDigitList();
          else {
              RecoDigits=RecoClusters->at(i_cluster)->convexHull();
          }
      cluster_time = RecoClusters->at(i_cluster)->GetTime();
      if (verbose > 2) std::cout <<"ClusterDisplay tool: This Cluster's RecoDigits size: "<<RecoDigits.size()<<std::endl;

      

      draw_clusters(i_cluster);
  ...
  if (total_hits_pmts_above_thr < npmtcut) {
    Log("ClusterDisplay tool: Only "+std::to_string(total_hits_pmts_above_thr)+" PMTs hit above threshold. Required: "+std::to_string(npmtcut)+". Terminate this execute step.",v_message,verbose);
    return true;
   }
  ...
  if (!isData){ 
    ...
    if (EventType == "ANNIEEvent"){
      ...
      if(MCLAPPDHits){
        Log("ClusterDisplay Tool: Size of MCLAPPDhits = "+std::to_string(MCLAPPDHits->size()),v_message,verbose);
        ...
        for (std::pair<unsigned long, std::vector<MCLAPPDHit>>&& apair : *MCLAPPDHits){
          ...
          if(det==nullptr){
            Log("ClusterDisplay Tool: LAPPD Detector not found! (chankey = "+std::to_string(chankey)+")",v_warning,verbose);
            continue;
          }
	  ...
	  std::vector<MCLAPPDHit>& hits = apair.second;
          Log("ClusterDisplay tool: Loop over MCLAPPDHits: Detector Key = "+std::to_string(detkey),v_message,verbose);
          for (MCLAPPDHit& ahit : hits){
            ...
            if ((lappds_selected && active_lappds.count(detkey)) || !lappds_selected){
              Log("ClusterDisplay tool: Loop over MCLAPPDHits: Detector Key LAPPD = "+std::to_string(detkey),v_debug,verbose);
              Log("ClusterDisplay tool: Loop over MCLAPPDHits: LAPPD hit at x="+std::to_string(x_lappd)+", y="+std::to_string(y_lappd)+", z="+std::to_string(z_lappd),v_debug,verbose);
        ...
        Log("ClusterDisplay tool: No MCLAPPDHits!", v_warning, verbose);
        num_lappds_hit = 0;
      }
    }
  } else {
    ...
    if(LAPPDHits){
      Log("ClusterDisplay Tool: Size of MCLAPPDhits = "+std::to_string(LAPPDHits->size()),v_message,verbose);
      ...
      for (std::pair<unsigned long, std::vector<LAPPDHit>>&& apair : *LAPPDHits){
        ...
        Log("ClusterDisplay tool: Loop over LAPPDHits: Detector Key = "+std::to_string(detkey),v_message,verbose);
        for (LAPPDHit& ahit : hits){
          ...
          double t_lappd = ahit.GetTime();
          if ((lappds_selected && active_lappds.count(detkey)) || !lappds_selected){
            Log("ClusterDisplay tool: Loop over LAPPDHits: Detector Key LAPPD = "+std::to_string(detkey),v_debug,verbose);
            Log("ClusterDisplay tool: Loop over LAPPDHits: LAPPD hit at x="+std::to_string(x_lappd)+", y="+std::to_string(y_lappd)+", z="+std::to_string(z_lappd),v_debug,verbose);
            Position lappd_hit(x_lappd,y_lappd,z_lappd);
            if (hits_LAPPDs.count(detkey)==0) hits_LAPPDs.emplace(detkey,std::vector<Position>{});
            hits_LAPPDs.at(detkey).push_back(lappd_hit);
            charge_lappd[detkey]+=1;
      ...
      Log("ClusterDisplay tool: No LAPPDHits!", v_warning, verbose);
      num_lappds_hit = 0;
    }
  }
  ...
  for (auto&& ahisto : charge_LAPPDs){
    ...
    if (it != lappd_detkeys.end()) histkey = std::distance(lappd_detkeys.begin(), it);
      ...
      Log("ClusterDisplay tool: Filling LAPPD histograms: Detkey " +std::to_string(detkey) +" not Found in Geometry class for LAPPDs!",v_error,verbose);
      continue;
    }*/
  ...
  Log("ClusterDisplay tool: Maximum time LAPPDs: "+std::to_string(maximum_time_lappds)+", minimum time LAPPDs: "+std::to_string(min_time_lappds),v_message,verbose);
  ...
  if (!isData){
    if(!TDCData){
      Log("ClusterDisplay tool: No TDC data to plot in Event Display!",v_message,verbose);
    } else {
      if(TDCData->size()==0){
        Log("ClusterDisplay tool: No TDC hits to plot in Event Display!",v_message,verbose);
      } else {
        Log("ClusterDisplay tool: Looping over FACC/MRD hits...Size of TDCData hits (MC): "+std::to_string(TDCData->size()),v_message,verbose);
    ...
    Log("ClusterDisplay tool: Looping over MRD data",v_message,verbose);
    // Loop over data file
    if (get_mrd){
    if(!TDCData_Data){
      Log("ClusterDisplay tool: No TDC data to plot in Event Display!",v_message,verbose);
      ...
      if(TDCData_Data->size()==0){
        Log("ClusterDisplay tool: No TDC hits to plot in Event Display!",v_message,verbose);
          ...
          Log("ClusterDisplay tool: Looping over FACC/MRD hits...Size of TDCData hits (data): "+std::to_string(TDCData_Data->size()),v_message,verbose);
  ...
  if (user_input){
    ...
    if (!isData){
      ...
      std::cout <<"P: ClusterDisplay for Previous Event (previous event: "<<mcevnum-1<<")"<<std::endl;
      std::cout <<"N: ClusterDisplay for Next Event (next event: "<<mcevnum+1<<")"<<std::endl;
    } else {
      std::cout <<"End of current event ("<<evnum<<"). Action? ----> "<<std::endl;
      std::cout <<"P: ClusterDisplay for Previous Event (previous event: "<<evnum-1<<")"<<std::endl;
      std::cout <<"N: ClusterDisplay for Next Event (next event: "<<evnum+1<<")"<<std::endl;
    }
    std::cout <<"Exxxx: ClusterDisplay for Event xxxx"<<std::endl;
    std::cout <<"Q: Quit ClusterDisplay ToolChain"<<std::endl;
    std::cin >> in_string;
...
bool ClusterDisplay::Finalise(){
  ...
  Log("ClusterDisplay tool: Clear canvas",v_debug,verbose);
  canvas_ev_display->Clear();
  Log("ClusterDisplay tool: Close canvas",v_debug,verbose);
  canvas_ev_display->Close();

  Log("ClusterDisplay tool: Terminate & Delete TApplication",v_debug,verbose);
  if (use_tapplication) {
    app_event_display->Terminate();
    delete app_event_display;
  }
  Log("ClusterDisplay tool: Delete additional histograms",v_debug,verbose);
  if (draw_histograms){
    delete leg_charge;
    delete leg_time;
  }
  Log("ClusterDisplay tool: Delete canvases",v_debug,verbose);
  ...
  Log("ClusterDisplay tool: Delete root file",v_debug,verbose);
  if (output_format == "root"){
    root_file->Close();
    delete root_file;
  }
...
void ClusterDisplay::make_gui(){

    //setup GUI for plotting Event Displays

    delete_canvas_contents();

    Log("ClusterDisplay tool: Make gui, Bird index = "+std::to_string(Bird_Idx),v_debug,verbose);
...
void ClusterDisplay::draw_event(int clusterNumber){
    ...
    draw_event_PMTs();
    Log("ClusterDisplay tool: Drawing PMTs finished.",v_message,verbose);

    draw_event_LAPPDs();
    Log("ClusterDisplay tool: Drawing LAPPDs finished.",v_message,verbose);

    draw_event_MRD();
    Log("ClusterDisplay tool: Drawing MRD finished.",v_message,verbose);

    if (text_box) draw_cluster_box(clusterNumber);
    Log("ClusterDisplay tool: Drawing event box finished.",v_message,verbose);

    draw_pmt_legend();
    Log("ClusterDisplay tool: Drawing PMT legend finished.",v_message,verbose);

    if (!isData) draw_lappd_legend();
    Log("ClusterDisplay tool: Drawing lappd legend finished.",v_message,verbose);

    draw_mrd_legend();
    Log("ClusterDisplay tool: Drawing mrd legend finished.",v_message,verbose);

    draw_schematic_detector();
    Log("ClusterDisplay tool: Drawing schematic detector finished.",v_message,verbose);

    if (!isData) draw_particle_legend();
    Log("ClusterDisplay tool: Drawing particle legend finished.",v_message,verbose);
    ...
    if (draw_vertex_temp) {
      ...
      if (n_particles_ring.at(i_particle)==1) {
        ...
        Log("ClusterDisplay tool: Drawing true vertices finished.",v_message,verbose);
    }

    if (draw_ring_temp) {
      for (unsigned int i_particle = 0; i_particle < n_particles_ring.size(); i_particle++){
	if (n_particles_ring.at(i_particle)==1) {
	  Log("ClusterDisplay tool: Drawing ring: particle # "+std::to_string(i_particle+1)+", truevtx = ("+std::to_string(particles_truevtx.at(i_particle).X())+","+std::to_string(particles_truevtx.at(i_particle).Y())+","+std::to_string(particles_truevtx.at(i_particle).Z())+"), true dir = ("+std::to_string(particles_truedir.at(i_particle).X())+","+std::to_string(particles_truedir.at(i_particle).Y())+","+std::to_string(particles_truedir.at(i_particle).Z())+"), color = "+std::to_string(particles_color.at(i_particle)),v_debug,verbose);
    ...
    //Log("ClusterDisplay tool: Drawing charge barycenter finished.",v_message,verbose);

    Log("ClusterDisplay tool: Drawing true ring finished.",v_message,verbose);

}

void ClusterDisplay::draw_cluster_box(int clusterNumber){
    ...
    std::string cluster_label = "Cluster #"+to_string(clusterNumber);
    ...
    std::string annie_time = "Cluster Time: ";
    std::string annie_time_unit = " [us]";
    std::string cluster_time_unit = " ns";
    //std::string annie_date = "Date: ";
    std::string mode_str = "Cluster mode: "+to_string(RecoClusters->at(clusterNumber)->GetClusterMode());
    double clusterCB = CBCheck(RecoDigits);
    std::string cb_str = "Cluster CB: "+to_string(clusterCB)+", "+to_string(RecoClusters->at(clusterNumber)->GetCB());
    double AS0 = ASCheck(RecoDigits,0);
    double AS1 = ASCheck(RecoDigits,1);
    std::string as_str = "Angular_span 0,1: "+to_string(AS0)+", "+to_string(AS1);
    std::string particle_label= "BestParentPDG: "+to_string(RecoClusters->at(clusterNumber)->GetPDG());
    std::string effpur_label="efficiency,purity: "+to_string(RecoClusters->at(clusterNumber)->Efficiency())+", "+to_string(RecoClusters->at(clusterNumber)->Purity());
    ...
    Log("Total PMT Hits: "+to_string(total_hits_pmts),v_debug,verbose);
    ...
    if (!isData) annie_time_label = annie_time+cluster_time_number+annie_time_unit;
    ...
    text_event_info->AddText((annie_event_label+" "+cluster_label).c_str());
    //text_event_info->AddText(cluster_label.c_str());
    text_event_info->AddText(mode_str.c_str());
    text_event_info->AddText(particle_label.c_str());
    text_event_info->AddText(effpur_label.c_str());
    ...
    text_event_info->AddText(cb_str.c_str());
    text_event_info->AddText(as_str.c_str());
  ...
  void ClusterDisplay::draw_pmt_legend(){
  ...
  void ClusterDisplay::draw_lappd_legend(){
  ...
  void ClusterDisplay::draw_mrd_legend(){
  ...
  void ClusterDisplay::draw_event_PMTs(){

    //draw PMT event markers

    Log("ClusterDisplay tool: Draw PMT event markers.",v_message,verbose)
    ;
    ...
    for (int i_pmt=0;i_pmt<n_tank_pmts;i_pmt++){
      ...
      if (fabs(y_pmt[detkey]-max_y)<0.01){
        ...
        if (mode == "Charge") {
            color_marker = Bird_Idx+int((charge[detkey]-threshold)/(maximum_pmts-threshold)*254);
            double effectivecharge = (charge[detkey] - threshold) / (maximum_pmts - threshold);
            Log("ClusterDisplay: (charge-threshold)/(max-threshold) ratio: " + to_string(effectivecharge), v_debug, verbose);
        }
        ...
        if (phi<-TMath::Pi() || phi>TMath::Pi())  Log("ClusterDisplay tool: Drawing Event: Phi out of bounds! X= "+std::to_string(x_pmt[detkey])+", y="+std::to_string(y_pmt[detkey])+", z="+std::to_string(z_pmt[detkey]),v_warning,verbose);
        ...
        if(charge[detkey]<threshold) Log("ATTENTION: FALSE HIT! "+to_string(detkey),v_debug,verbose);
        if (mode == "Charge") {
            color_marker = Bird_Idx+int((charge[detkey]-threshold)/(maximum_pmts-threshold)*254);
            double effectivecharge = (charge[detkey] - threshold) / (maximum_pmts - threshold);
            Log("ClusterDisplay: (charge-threshold)/(max-threshold) ratio: " + to_string(effectivecharge), v_debug, verbose);
    ...
    Log("Total PMT Markers: "+to_string(marker_pmts_top.size()+ marker_pmts_bottom.size()+ marker_pmts_wall.size()),v_debug,verbose);
  ...
  void ClusterDisplay::draw_event_LAPPDs(){

      //draw LAPPD markers on Event Display
      Log("ClusterDisplay tool: Drawing LAPPD event markers.",v_message,verbose);
      ...
      for(auto&& these_lappd_hit_positions : hits_LAPPDs){
        ...
        for (unsigned int i_single_lappd=0; i_single_lappd<these_lappd_hit_positions.second.size(); i_single_lappd++){
          ...
          phi-=TMath::Pi();
          if (phi < - TMath::Pi()) phi = -TMath::Pi();
          if (phi < - TMath::Pi() || phi>TMath::Pi())  Log("ClusterDisplay tool: Drawing LAPPD event: ERROR: Phi out of bounds! X= "+std::to_string(x_lappd)+", y="+std::to_string(y_lappd)+", z="+std::to_string(z_lappd),v_warning,verbose);
          x[0]=0.5+phi*size_top_drawing;
          ...
          if (mode == "Charge") color_marker = Bird_Idx+254;      //FIXME in case of actual charge information for LAPPDs
            ...
            if (time_lappd_single > maximum_time_overall || time_lappd_single < min_time_overall) Log("ClusterDisplay tool: Error: LAPPD hit time out of bounds! LAPPD time = "+std::to_string(time_lappd_single)+", min time: "+std::to_string(min_time_overall)+", max time: "+std::to_string(maximum_time_overall),v_error,verbose);
  ...
  void ClusterDisplay::draw_event_MRD(){

    //draw MRD hits on Event Display

    Log("ClusterDisplay tool: Drawing MRD hits.",v_message,verbose);
  ...
  void ClusterDisplay::draw_true_vertex(Position trueVtx, Direction trueDir, int ring_color){
    ...
    std::string logmessage = "ClusterDisplay tool: Projected true vertex on wall: ("+std::to_string(vtxproj_x)+", "+std::to_string(vtxproj_y)+", "+std::to_string(vtxproj_z)+"). Drawing vertex on ClusterDisplay.";
    Log(logmessage,v_message,verbose);
  ...
  void ClusterDisplay::draw_barycenter(Position posBary){
  ...
  //quaternial structs as help objects for rotating vectors around vectors

  struct quaternial {             //for rotations of vectors around vectors

    double q0, q1, q2, q3;

  };

  quaternial multiplication(quaternial Q1, quaternial Q2){
  ...
  quaternial devine_rotationAxis(double angle, double x, double y, double z){

    quaternial Q_Rotation;
  ...
  quaternial invert_quaternial(quaternial Q1){
  ...
  void ClusterDisplay::draw_true_ring(Position trueVtx, Direction trueDir, int ring_color){
    ...
    */
    ...
    for (int i_ring=0;i_ring<num_ring_points;i_ring++){
      ...
      quaternial Q_RotAxis = devine_rotationAxis(phi_ring, truedirX, truedirY, truedirZ);
      quaternial Q_RotAxis_Inverted = invert_quaternial(Q_RotAxis);
      quaternial Q_Intermediate = multiplication(Q_RotAxis, Q_RingStart);
      quaternial Q_Rotated = multiplication(Q_Intermediate, Q_RotAxis_Inverted);
      ...
      Log("ClusterDisplay tool: Drawing Ring: Ring point # "+std::to_string(i_ring)+", projected x/y/z = "+std::to_string(vtxproj_x2)+"/"+std::to_string(vtxproj_y2)+"/"+std::to_string(vtxproj_z2),v_message,verbose);
      ...
      if (status_pre == status_ev){
          if (status_ev == 3) {       //for wall part need to take care of boundary events
            if (i_angle == 0) {
              ...
              phi_post = phi_calc;
              Log("ClusterDisplay tool: Drawing Ring: Phi pre = "+std::to_string(phi_pre)+", phi post = "+std::to_string(phi_post),v_message,verbose);
              if (fabs(phi_post-phi_pre) > TMath::Pi()/2.) {
                i_ring_area++;
                firstev_area[i_ring_area] = i_valid_points;
              }  
              Log("ClusterDisplay tool: Drawing Ring: Ring area # "+std::to_string(i_ring_area),v_message,verbose);
              phi_pre = phi_post;
            }
          }
  ...
  void ClusterDisplay::find_projected_xyz(double vtxX, double vtxY, double vtxZ, double dirX, double dirY, double dirZ, double &projected_x, double &projected_y, double &projected_z){
    ...
    Log("ClusterDisplay tool: Find projected xyz: dirY = "+std::to_string(dirY)+", max_y = "+std::to_string(max_y)+", min_y = "+std::to_string(min_y)+", vtxY = "+std::to_string(vtxY)+", vtxX = "+std::to_string(vtxX)+", dirX = "+std::to_string(dirX)+", vtxZ = "+std::to_string(vtxZ)+",dirZ = "+std::to_string(dirZ),v_debug,verbose);
    ...
    Log("ClusterDisplay tool: Find projected xyz: a= "+std::to_string(a)+", b= "+std::to_string(b)+", c= "+std::to_string(c),v_debug,verbose);
    ...
    Log("ClusterDisplay tool: Find projected xyz: Time_min: "+std::to_string(time_min)+", time wall1: "+std::to_string(time_wall1)+", time wall2: "+std::to_string(time_wall2)+", time wall: "+std::to_string(time_wall)+", time top: "+std::to_string(time_top),v_debug,verbose);
  ...
  void ClusterDisplay::translate_xy(double vtxX, double vtxY, double vtxZ, double &xWall, double &yWall, int &status_hit, double &phi_calc){
  ...
  void ClusterDisplay::delete_canvas_contents(){

    //delete all objects that were drawn on the canvas so far

    Log("ClusterDisplay tool: Delete canvas contents.",v_message,verbose);
    ...
    Log("ClusterDisplay tool: Current number of polylines (to be deleted): "+std::to_string(current_n_polylines),v_message,verbose);
  ...
  void ClusterDisplay::draw_schematic_detector(){
...
void ClusterDisplay::draw_particle_legend(){
...
void ClusterDisplay::set_color_palette(){
...
void ClusterDisplay::ParseUserInput(std::string user_string){
  ...
  if (firstChar == 'P') {
    ...
    if (new_evnum < 0) {
      Log("ClusterDisplay tool: Already at the first event! Cannot select previous event",v_error,verbose);
      m_data->CStore.Set("UserEvent",false);
    } else {
      Log("ClusterDisplay tool: Going to event nr "+std::to_string(new_evnum),v_message,verbose);
      m_data->CStore.Set("UserEvent",true);
    ...
    Log("ClusterDisplay tool: Conversion "+user_string+"--> Event Number "+std::to_string(new_evnum),v_debug,verbose);
    Log("ClusterDisplay tool: Going to event nr "+std::to_string(new_evnum),v_message,verbose);
    m_data->CStore.Set("UserEvent",true);
    m_data->CStore.Set("LoadEvNr",new_evnum);
    m_data->CStore.Set("CheckFurtherTriggers",false);
    ...
    Log("ClusterDisplay tool: Quitting ClusterDisplay toolchain.",v_message,verbose);
    m_data->CStore.Set("UserEvent",false);
    m_data->vars.Set("StopLoop",1);
  }
  else {
    Log("ClusterDisplay tool: User Input "+user_string+" not recognized. Please make sure you specified everything in the right format.",v_error,verbose);
    Log("ClusterDisplay tool: Getting next event instead",v_error,verbose);
    m_data->CStore.Set("UserEvent",false);
  }
...
double ClusterDisplay::CBCheck(std::vector<RecoDigit*> digits) {
    ...
    if (verbose > 4) std::cout << "ClusterDisplay Tool: CB: " << charge_balance << std::endl;

    return charge_balance;
}

double ClusterDisplay::ASCheck(vector<RecoDigit*> digits, int mode) {
...
void ClusterDisplay::draw_clusters(int clusterNumber) {
  ...
  //---------------------------------------------------------------
    ...
    Log("ClusterDisplay tool: Drawing Cluster Display.", v_message, verbose);

    canvas_ev_display->Clear();
    canvas_ev_display->Divide(1, 1);
    make_gui();
    draw_event(clusterNumber);
    std::stringstream ss_evdisplay_title, ss_evdisplay_name;
    if (!isData) {
        ss_evdisplay_title << "evdisplay_ev" << mcevnum << "_cl"<< clusterNumber << MCTriggernum;
        ss_evdisplay_name << "canvas_evdisplay_ev" << mcevnum << "_cl"<< clusterNumber << MCTriggernum;
    }
    else {
        ss_evdisplay_title << "evdisplay_ev" << evnum <<"_cl"<<clusterNumber;
        ss_evdisplay_name << "canvas_evdisplay_ev" << evnum <<"_cl"<<clusterNumber;
    }
    ...
    if (draw_histograms) {
        ...
        if (!isData) {
            ss_pmt_title << "pmt_ev" << mcevnum << "_" << MCTriggernum<<"_cl"<<clusterNumber;
            ss_pmt_name << "canvas_pmt_ev" << mcevnum << "_" << MCTriggernum<<"_cl"<<clusterNumber;
        }
        else {
            ss_pmt_title << "pmt_ev" << evnum<<"_cl"<<clusterNumber;
            ss_pmt_name << "canvas_pmt_ev" << evnum<<"_cl"<<clusterNumber;
        }
        ...
        if (!isData) {
            ss_pmt2D_title << "pmt2D_ev" << mcevnum << "_" << MCTriggernum<<"_cl"<<clusterNumber;
            ss_pmt2D_name << "canvas_pmt2D_ev" << mcevnum << "_" << MCTriggernum << "_cl" << clusterNumber;
        }
        else {
            ss_pmt2D_title << "pmt2D_ev" << evnum << "_cl" << clusterNumber;
            ss_pmt2D_name << "canvas_pmt2D_ev" << evnum << "_cl" << clusterNumber;
        }
    ...
    if (save_plots) {
        if (output_format == "image") canvas_ev_display->SaveAs(filename_evdisplay.c_str());
            ...
            Log("ClusterDisplay tool: Saving canvas to root-file", v_message, verbose);
            
            if(RecoClusters->at(clusterNumber)->GetTime()<10000) {
                if(RecoClusters->at(clusterNumber)->GetPDG()==13) root_file->cd("prompt/Muon");
                else if(RecoClusters->at(clusterNumber)->GetPDG() == 2112) root_file->cd("prompt/Neutron");
                else if(RecoClusters->at(clusterNumber)->GetPDG() == 2212) root_file->cd("prompt/Proton");
                else root_file->cd("prompt/other");
            }
            else {

                if (RecoClusters->at(clusterNumber)->GetPDG() == 13) root_file->cd("delayed/Muon");
    else if (RecoClusters->at(clusterNumber)->GetPDG() == 2112) root_file->cd("delayed/Neutron");
    else if (RecoClusters->at(clusterNumber)->GetPDG() == 2212) root_file->cd("delayed/Proton");
    else root_file->cd("delayed/other");
            }
...
void ClusterDisplay::collect_digits() {
    ...
    for (unsigned int i_digit = 0; i_digit < RecoDigits.size(); i_digit++) {
        ...
        if (digittype == 0) {
            ...
            Log("ClusterDisplay tool: Reading in RecoEvent data: PMTid = " + std::to_string(pmtid) + ", chankey = " + std::to_string(chankey) + ", detkey = " + std::to_string(detkey), v_debug, verbose);
            Log("ClusterDisplay tool: Reading in RecoEvent data: DigitQ = " + std::to_string(digitQ) + ", digitT = " + std::to_string(digitT), v_debug, verbose);
            ...
            if (verbose > 2) std::cout << "ClusterDisplay tool: passed_lower_time_cut: " << passed_lower_time_cut << ", passed_upper_time_cut: " << passed_upper_time_cut << std::endl;
            if (digitQ >= threshold && passed_lower_time_cut && passed_upper_time_cut) {
                ...
                hitpmt_detkeys.push_back(detkey);
                total_hits_pmts++;
                if (verbose > 2) std::cout << "ClusterDisplay tool: Filling maps" << std::endl;
            }
        }
        else if (digittype == 1) {
        ...
        int chrg_count=0;
        for (std::pair<unsigned long, double>apair : charge) {
            if(apair.second != 0) chrg_count++;
        }
        Log("Total charge entries: "+to_string(chrg_count),v_debug,verbose);
