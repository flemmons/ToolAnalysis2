...
bool FindNeutrons::Execute(){
  ...
  if (get_ok){
    ...
    for (int i_neutron=0; i_neutron < vec_neutrons.size(); i_neutron++){
      ...
      if (i_neutron == 0) neutStartIndex = Particles_Temp.size();
    }
  } else {
  ...
  m_data->Stores["ANNIEEvent"]->Set("neutStartIndex", neutStartIndex);
...
bool FindNeutrons::FindNeutronsByCB(bool strict){
  ...
  for (std::map<double,double>::iterator it = ClusterTotalPEs.begin(); it!= ClusterTotalPEs.end(); it++){
    ...
    if (it->first > 10000){
      ...
      else if (verbosity > 4) Log("FindNeutrons tool: No Neutron candidate at cluster # " + std::to_string(tmp_cluster_id) + ", time: " + std::to_string(it->first) + " ns...\nq: " + std::to_string(current_q) + " pe\n CB: " + std::to_string(current_cb), v_debug, verbosity);
    }
    tmp_cluster_id ++;
  }
...
bool FindNeutrons::FillRecoParticles(){
  ...
  std::cout << "Note the cluster times size: " << cluster_times_neutron.size() << endl;
