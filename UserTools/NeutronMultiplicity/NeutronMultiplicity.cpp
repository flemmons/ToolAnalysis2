...
bool NeutronMultiplicity::Initialise(std::string configfile, DataModel &data){
  ...
  if (save_bs){
      if(!save_root&&IsMC) truevtx=new RecoVertex();
...
bool NeutronMultiplicity::Execute(){
  ...
  //Get (MC)Particles variable
  bool get_ok = false;
  //if (isMC)get_ok = m_data->Stores["ANNIEEvent"]->Get("MCParticles", Particles);
  /*else*/ get_ok = m_data->Stores["ANNIEEvent"]->Get("Particles",Particles);
  m_data->Stores["ANNIEEvent"]->Get("neutStartIndex", neutStartIndex);
  ...
  NumberNeutrons = 0;
  if (get_ok){
    for (int i_particle=neutStartIndex; i_particle < (int) Particles.size(); i_particle++){
      ...
      if (Particles.at(i_particle).GetPdgCode() == 2112) {
         NumberNeutrons++;
         Log("HEY IT'S A NEUTRON!"+std::to_string(NumberNeutrons),2,verbosity);
      }
    }
  }
  ...
  if (pass_selection && pass_reconstruction){
    ...
    if (isMC) {
        this->GetMCTruthInformation();
        Log("Substituting true energy" + to_string(true_Emu) + " for reconstructed.", v_debug, verbosity);
        SimpleRecoEnergy = true_Emu;
        Log("New SimpleRecoEnergy: " + to_string(SimpleRecoEnergy), v_debug, verbosity);
    }

    //Fill ROOT histograms
    std::cout << "save_root " << save_root << endl;
...
bool NeutronMultiplicity::InitialiseHistograms(){
  ...
  dir_neutron->cd();
  h_neutrons = new TH1F("h_neutrons","Number of neutrons in beam events",1500,0,1500);
  h_neutrons_mrdstop = new TH1F("h_neutrons_mrdstop","Number of of neutrons in beam events (MRD stop)",15,0,15);
  h_neutrons_mrdstop_fv = new TH1F("h_neutrons_mrdstop_fv","Number of of neutrons in beam events (MRD stop, FV)",15,0,15);
...
bool NeutronMultiplicity::FillHistograms(){
    ...
    Log("NeutronMultiplicity Tool: Filling Histograms", v_debug, verbosity);
    Log("SimpleRecoEnergy: " + to_string(SimpleRecoEnergy), v_debug, verbosity);
  ...
  if (verbosity > 1){
    ...
    std::cout << "SimpleRecoEnergy: " << SimpleRecoEnergy << endl;
  }
...
bool NeutronMultiplicity::GetParticleInformation(){
  ...
  else Log("SimpleRecoEnergy found!  Value: " + std::to_string(SimpleRecoEnergy),v_debug,verbosity);
...
bool NeutronMultiplicity::GetMCTruthInformation(){
  ...
  Log("true_Enu: " + to_string(true_Enu),v_debug,verbosity);
  ...
  Log("MCNeutCap count of CaptVtxX: " + std::to_string(MCNeutCap.count("CaptVtxX")), v_debug, verbosity);
  ...
  if (MCNeutCap.count("CaptVtxX")>0){
    ...
    Log("n_vtxx size: " + std::to_string(n_vtxx.size()), vv_debug, verbosity);
    Log("n_vtxy size: " + std::to_string(n_vtxy.size()), vv_debug, verbosity);
    Log("n_vtxz size: " + std::to_string(n_vtxz.size()), vv_debug, verbosity);
    Log("n_parent size: " + std::to_string(n_parent.size()), vv_debug, verbosity);
    Log("n_ngamma size: " + std::to_string(n_ngamma.size()), vv_debug, verbosity);
    Log("n_totale size: " + std::to_string(n_totale.size()), vv_debug, verbosity);
    Log("n_time size: " + std::to_string(n_time.size()), vv_debug, verbosity);
    Log("n_nuc size: " + std::to_string(n_nuc.size()), vv_debug, verbosity);
    Log("n_primary size: " + std::to_string(n_primary.size()), vv_debug, verbosity);
...
bool NeutronMultiplicity::FillSingleTGraph(TGraphErrors *gr, TH2F *h2d, std::vector<std::string> labels){
    ...
    Log("NeutronMultiplicity::FillSingleTGraph running", v_debug, verbosity);
  ...
  Log("n_bins x,y: " + to_string(n_bins_x) + " , " + to_string(n_bins_y), v_debug, verbosity);
